...
namespace ChangeExcel
{
    class ExcelWorker
    {
        ...
        //Reading From Excel
        public BindingList<Tempo> ReadExcel(int col)
        {
            ...
            try
            {
                for (int index = 2; index <= lastRow; index++)
                {
                    ...
                    {
                        key = MyValues.GetValue(1, col).ToString(),
                        value = ""
                    });
                }
            }
        ...
        public void WriteExcel(IList<Tempo> listJsonObj, string column)
        {
            try
            {
                 for (int index = 2; index <= lastRow; index++)
                {
                    ...
                    {
                        if ((!string.IsNullOrEmpty(myValues.GetValue(1, 1).ToString().Trim())) && (num.key.Trim() == myValues.GetValue(1, 1).ToString().Trim()))
                        {
