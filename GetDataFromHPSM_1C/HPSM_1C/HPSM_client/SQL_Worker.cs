...
namespace HPSM_client
{
    class SQL_Worker
    {
        ...
        public static string _createTable_HPSMLabor = @"BEGIN EXECUTE IMMEDIATE 'CREATE TABLE " + HPSMTables.HPSMLabor + @"(ID NUMBER(10) NOT NULL, I_NUMBER VARCHAR(50) NOT NULL, DT DATE NOT NULL, TIME_SPEND FLOAT NOT NULL, FIO VARCHAR(200) NOT NULL, KE VARCHAR(500) NULL, AddedDate DATE NOT NULL, CONSTRAINT ID_pk PRIMARY KEY(ID))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE = -955 THEN NULL; ELSE RAISE; END IF; END;";


        public static string _insert_HPSMLabor = @"BEGIN EXECUTE IMMEDIATE 'INSERT INTO " + HPSMTables.HPSMLabor + @" VALUES ({0},''{1}'', TO_DATE(''{2}'', ''DD.MM.YYYY''), ROUND({3}/60, 2),''{4}'', ''{5}'', TO_DATE(''{6}'', ''DD.MM.YYYY''))'; END;";
