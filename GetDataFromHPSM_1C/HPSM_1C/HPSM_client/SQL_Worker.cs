...
namespace HPSM_client
{
    class SQL_Worker
    {
        ...
        public static string _insert_HPSMLabor = @"BEGIN EXECUTE IMMEDIATE 'INSERT INTO " + HPSMTables.HPSMLabor + @" VALUES ({0},''{1}'', TO_DATE(''{2}'', ''DD.MM.YYYY''), ROUND(TO_BINARY_FLOAT(REPLACE(''{3}'','','',''.''))/60, 2),''{4}'', ''{5}'', ''{6}'', TO_DATE(''{7}'', ''DD.MM.YYYY''))'; END;";
