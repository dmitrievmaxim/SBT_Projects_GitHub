...
namespace GetDataFromJIRAStructure
{
   ...
   SET item_1 = item_2, item_3 = item_4, item_5 = item_6, item_7 = item_8, item_9 = item_10, item_11 = item_12, item_13 = item_14, item_15 = item_16, item_17 = item_18, item_19 = item_20, item_21 = item_22, item_23 = item_24, item_25 = item_26, item_27 = item_28";
        ...
        public static string _delete_Unused_TempoLabor = @"DELETE FROM TEMPOLABOR WHERE (WORKDATE BETWEEN TO_DATE('{0}', 'DD.MM.YY') AND TO_DATE('{1}', 'DD.MM.YY')) AND ID_TIMESHEET IN (SELECT labor.ID_TIMESHEET FROM TEMPOLABOR labor LEFT JOIN TEMPOLABORTMP tmp ON labor.ID_TIMESHEET=tmp.ID_TIMESHEET WHERE tmp.ID_TIMESHEET IS NULL AND (labor.WORKDATE BETWEEN TO_DATE('{0}', 'DD.MM.YY') AND TO_DATE('{1}', 'DD.MM.YY')))";
