...
namespace GetDataFromJIRAStructure
{
    class SQL_Worker
    {
        ...
        public static string _createTable_TempoLaborTmp = @"BEGIN EXECUTE IMMEDIATE 'CREATE TABLE " + TempoTables.TempoLaborTmp + @"(ID NUMBER(10) NOT NULL, ID_timesheet NUMBER(10) NOT NULL, ID_issue NUMBER(10) NOT NULL, Issue_name varchar(30) NOT NULL, Summary varchar(1000) NOT NULL, Timespent FLOAT NOT NULL, Workdate DATE NOT NULL, Full_name varchar(200) NOT NULL, User_name varchar(100) NOT NULL, Issue_type varchar(100) NOT NULL, ID_project NUMBER(10) NOT NULL, Description varchar(4000) NULL, Work_type_val varchar(1000) NULL, As_num varchar(1000) NULL, As_val varchar(1000) NULL, CONSTRAINT ID_timesheet_tmp_pk PRIMARY KEY(ID_timesheet))'; EXCEPTION WHEN OTHERS THEN IF SQLCODE = -955 THEN NULL; ELSE RAISE; END IF; END;";


        public static string _insert_TempoLaborTmp = @"BEGIN EXECUTE IMMEDIATE 'INSERT INTO " + TempoTables.TempoLaborTmp + @" VALUES ({0},{1},{2},''{3}'',''{4}'',ROUND({5}/3600,2), TO_DATE(''{6}'', ''DD.MM.YYYY''),''{7}'',''{8}'',''{9}'',{10},''{11}'',''{12}'',''{13}'',''{14}'')'; END;";
